...
if ( ! function_exists('grupo_select'))
{
	/**
	 * Creates a country-based dropdown form input based on the entries in the address.countries config.
	 *
	 ...
	 * @return string The full html for the select input.
	 */
	function grupo_select($selected_iso='', $default_iso='0', $select_name='cod_grupo', $class='' )
	{
		// First, grab the states from the config
		/*$countries = config_item('address.countries');
		...
		$sql='SELECT "CTA Grupos de Clientes".CODIGO, "CTA Grupos de Clientes".DESCRIP
		FROM "CTA Grupos de Clientes" "CTA Grupos de Clientes"';
		...
		$output  = '<select name="'. $select_name .'" id="' . $select_name . '" ' . $class . ' >';
		$output .= '<option value="0">Todos</option>' . PHP_EOL;
		while ($row = odbc_fetch_object($result))
		{
			$output .= "<option value='{$row->CODIGO}'";
			$output .= ($row->CODIGO == $selected_iso) ? ' selected="selected"' : '';
			$output .= empty($selected_iso) && ($default_iso == $row->CODIGO) ? ' selected="selected"' : '';
			$output .= ">".utf8_encode($row->DESCRIP)."</option>\n";
		}
		$output .= "</select>\n";

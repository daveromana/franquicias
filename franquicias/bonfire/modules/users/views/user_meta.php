		<?php foreach ($meta_fields as $field):?>
			...
			<?php elseif ($field['form_detail']['type'] == 'state_select' && is_callable('state_select')) : ?>
				...
				<?php elseif ($field['form_detail']['type'] == 'grupo_select' && is_callable('grupo_select')) : ?>

					<div class="control-group <?php echo iif( form_error('grupo') , 'error'); ?>">
						<label class="control-label" for="grupo">Grupos</label>
						<div class="controls">
							<?php echo grupo_select(set_value($field['name'], isset($user->$field['name']) ? $user->$field['name'] : 'US'), 'US', 'cod_grupo', 'span6 chzn-select'); ?>
